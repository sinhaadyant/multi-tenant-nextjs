...
export interface UserCredentials {
  ...
  tenantName?: string;
  tenantDomain?: string;
  permissions: Record<string, Record<string, {
    canCreate: boolean;
    canRead: boolean;
    canUpdate: boolean;
    canDelete: boolean;
...
export class BackendCredentialsHelper {
  private credentials: TestCredentials;

  constructor() {
    // Load credentials from the common credentials file
  ...
  getTenant(tenantKey: string): TenantInfo {
    ...
    if (!tenant) {
      throw new Error(`Tenant with key '${tenantKey}' not found in credentials`);
    }
    return tenant;
  }
  ...
  hasPermission(userKey: string, module: string, submodule: string, action: 'canCreate' | 'canRead' | 'canUpdate' | 'canDelete' | 'canViewAll'): boolean {
  ...
  canAccessSubmodule(userKey: string, module: string, submodule: string): boolean {
    const user = this.getUser(userKey);
    const modulePerms = user.permissions[module];
    if (!modulePerms) return false;
  ...
  getModulePermissions(userKey: string, module: string): Record<string, any> | null {
    const user = this.getUser(userKey);
  ...
  getTenantForUser(userKey: string): TenantInfo | null {
    ...
    return this.getAllTenants().find(tenant => tenant.id === user.tenantId) || null;
  }
  ...
  getUsersWithPermission(module: string, submodule: string, action: 'canCreate' | 'canRead' | 'canUpdate' | 'canDelete' | 'canViewAll'): UserCredentials[] {
  ...
  getUsersWithoutPermission(module: string, submodule: string, action: 'canCreate' | 'canRead' | 'canUpdate' | 'canDelete' | 'canViewAll'): UserCredentials[] {
    return this.getAllUsers().filter(user => {
      const modulePerms = user.permissions[module];
  ...
  // Test data generation helpers for backend API testing
  generateTestUser(overrides: Partial<UserCredentials> = {}): UserCredentials {
    ...
    return {
      ...
      permissions: {
        Dashboard: {
          Overview: { canCreate: false, canRead: true, canUpdate: false, canDelete: false, canViewAll: false },
          Analytics: { canCreate: false, canRead: true, canUpdate: false, canDelete: false, canViewAll: false }
        },
        "User Management": {
          "User List": { canCreate: false, canRead: false, canUpdate: false, canDelete: false, canViewAll: false },
          "User Profile": { canCreate: false, canRead: true, canUpdate: true, canDelete: false, canViewAll: false }
        },
        Support: {
          Tickets: { canCreate: true, canRead: true, canUpdate: false, canDelete: false, canViewAll: false },
          "Knowledge Base": { canCreate: false, canRead: true, canUpdate: false, canDelete: false, canViewAll: false }
        }
      },
      ...overrides
    };
  ...
  generateAPIUserData(overrides: any = {}): any {
    return {
  ...
  generateAPITenantData(overrides: any = {}): any {
    return {
      name: `API Test Tenant ${Date.now()}`,
      domain: `api-test-${Date.now()}.example.com`,
      slug: `api-test-${Date.now()}`,
      loginRestrictions: {
        allowedDomains: [`api-test-${Date.now()}.com`],
        maxUsers: 50
      },
      status: 'active',
      ...overrides
  ...
  generateAPILoginData(overrides: any = {}): any {
    return {
      email: `api-login-${Date.now()}@example.com`,
      password: 'TestPassword123!',
      tenantSlug: 'tenant-a',
      ...overrides
    };
  ...
  generateAPIErrorTestData(): any[] {
    return [
      { type: 'validation', data: { email: 'invalid-email' } },
      { type: 'authentication', data: { email: 'nonexistent@example.com', password: 'wrong' } },
      { type: 'authorization', data: { userId: 'invalid-id' } },
      { type: 'notFound', data: { resourceId: 'nonexistent-id' } },
      { type: 'rateLimit', data: { attempts: 20 } },
    ];
  }

  generateAPIPerformanceTestData(count: number = 100): any[] {
    const data: any[] = [];
    for (let i = 0; i < count; i++) {
      data.push({
        name: `Performance API User ${i + 1}`,
        email: `performance-api-${i + 1}@example.com`,
        password: 'TestPassword123!',
        role: 'Tenant User',
        metadata: {
          testId: i,
  ...
  generateAPIConcurrentTestData(count: number = 10): any[] {
    const data: any[] = [];
    for (let i = 0; i < count; i++) {
      data.push({
        name: `Concurrent API User ${i + 1}`,
        email: `concurrent-api-${i + 1}@example.com`,
        password: 'TestPassword123!',
        role: 'Tenant User',
        concurrentId: i,
      });
  ...
  generateAPIBulkTestData(count: number = 50): any[] {
    const data: any[] = [];
    for (let i = 0; i < count; i++) {
      data.push({
        name: `Bulk API User ${i + 1}`,
        email: `bulk-api-${i + 1}@example.com`,
        password: 'TestPassword123!',
        role: 'Tenant User',
        bulkId: i,
      });
...
export const backendCredentialsHelper = new BackendCredentialsHelper();
