...
class SuperAdminAuthTester {
  ...
  async runAllTests() {
    ...
    try {
      ...
      console.log(`✅ SuperAdmin Authentication Tests: ${this.testResults.passed} passed, ${this.testResults.failed} failed`);
  ...
  async testValidLogin() {
    try {
      ...
      await this.testHelper.typeText('input[name="password"]', credentials.password);
      ...
      await this.testHelper.page.waitForNavigation({ waitUntil: 'networkidle0' });
  ...
  async testInvalidLogin() {
    try {
      ...
      await this.testHelper.typeText('input[name="email"]', 'invalid@example.com');
      ...
      const errorElement = await this.testHelper.page.$('.error-message, .alert-error, [data-testid="error-message"]');
      if (errorElement) {
        const errorText = await errorElement.evaluate(el => el.textContent);
        console.log('    ✅ Invalid login properly rejected:', errorText);
        this.testResults.passed++;
        this.testResults.details.push('Invalid SuperAdmin login properly rejected');
      } else {
  ...
  async testForgotPassword() {
    try {
      ...
      await this.testHelper.page.goto('http://localhost:3000/superadmin/forgot-password');
      ...
      const successElement = await this.testHelper.page.$('.success-message, .alert-success, [data-testid="success-message"]');
      if (successElement) {
        ...
        this.testResults.passed++;
        this.testResults.details.push('SuperAdmin forgot password request successful');
      } else {
        throw new Error('No success message displayed for forgot password request');
      }
  ...
  async testPasswordReset() {
    try {
      ...
      await this.testHelper.page.goto('http://localhost:3000/superadmin/reset-password?token=test-token');
      ...
      await this.testHelper.typeText('input[name="confirmPassword"]', newPassword);
      ...
      if (successElement) {
        ...
        if (currentUrl.includes('/superadmin/login')) {
          ...
          this.testResults.passed++;
          this.testResults.details.push('SuperAdmin password reset successful - redirected to login');
        } else {
  ...
  async testInviteSuperAdmin() {
    try {
      ...
      await this.testHelper.page.goto('http://localhost:3000/superadmin/invite');
      ...
      // Submit form
      const inviteSubmitButton = await this.testHelper.page.$('button[type="submit"]');
      ...
      if (successElement) {
        ...
        this.testResults.passed++;
        this.testResults.details.push('SuperAdmin invite functionality working');
        
        // Verify in database
        const invitedAdmin = await this.dbHelper.getSuperAdminByEmail(inviteEmail);
        if (invitedAdmin) {
  ...
  async testInputValidations() {
    try {
      ...
      const emailError = await this.testHelper.page.$('.error-message, .alert-error, [data-testid="email-error"]');
      if (emailError) {
      ...
      const formatError = await this.testHelper.page.$('.error-message, .alert-error, [data-testid="email-format-error"]');
      if (formatError) {
      ...
      const passwordError = await this.testHelper.page.$('.error-message, .alert-error, [data-testid="password-error"]');
      if (passwordError) {
        console.log('    ✅ Password validation working');
        this.testResults.passed++;
      }
      
      this.testResults.details.push('SuperAdmin input validations working correctly');
  ...
  async testLogout() {
    try {
      ...
      const logoutButton = await this.testHelper.page.$('button[data-testid="logout"], .logout-btn, a[href*="logout"]');
      if (logoutButton) {
  ...
  async testSessionManagement() {
    try {
      ...
      if (currentUrl.includes('/superadmin/dashboard')) {
        ...
        this.testResults.passed++;
        this.testResults.details.push('SuperAdmin session management working correctly');
      } else {
