...
import { TenantUser } from '@/hooks/useTenantsAPI';
...
interface TenantUserManagementProps {
  ...
  onFiltersChange: (filters: Partial<TenantUserManagementProps['filters']>) => void;
...
const TenantUserManagement: React.FC<TenantUserManagementProps> = ({
  tenantId,
  users,
  userStats,
  pagination,
  ...
  const handleBulkAction = useCallback((action: 'activate' | 'deactivate' | 'delete') => {
    if (selectedUsers.length === 0) {
      toast.error('Please select users to perform bulk action');
      return;
    }

    const selectedUserObjects = processedUsers.filter(user => selectedUsers.includes(user.id));
    
    confirm({
      title: `Bulk ${action.charAt(0).toUpperCase() + action.slice(1)} Users`,
      ...
      onConfirm: () => {
        ...
        toast.success(`${action.charAt(0).toUpperCase() + action.slice(1)} action completed for ${selectedUsers.length} users`);
        setSelectedUsers([]);
      },
    });
  }, [selectedUsers, processedUsers, confirm, toast]);
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {filters.search || filters.status ? 'No users match your filters.' : 'No users found for this tenant.'}
            </p>
          </div>
        ) : (
          <>
