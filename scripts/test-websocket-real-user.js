...
async function testWebSocketRealUser() {
  const BASE_URL = 'http://localhost:3000';
  const USER_EMAIL = 'anil@cc.com';
  const USER_PASSWORD = 'password123';
  const TENANT_SLUG = 'riyo';

  console.log('🔌 Testing WebSocket Connection with Real User');
  console.log('=============================================');

  try {
    // Step 1: Login as the user to get real credentials
    ...
    // Step 2: Create WebSocket connection with real credentials
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const notificationResponse = await axios.post(`${BASE_URL}/api/superadmin/notifications`, {
          ...
          message: 'This is a test notification to verify WebSocket real-time delivery.',
          type: 'info',
          priority: 'medium',
